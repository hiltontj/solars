...
const useAppState = (): State => {
  ...
  if (state === null || state === undefined) {
    throw new Error('useAppState must be called within a valid AppStateContext.Provider');
  }
  return state;
}
...
const useAppDispatcher = (): AppDispatcher => {
  ...
  if (dispatch === null || dispatch === undefined) {
    throw new Error('useAppDispatcher must be called within a valid AppDispatchContext.Provider');
  }
  return dispatch;
}
...
export const useUpdateYear = (): (year: number) => void => {
  ...
  return React.useCallback((year: number) =>  dispatch({ tag: 'UpdateYear', year }), []);
}
